...
export function parseProjectFile(raw: string, slugFallback: string, absolutePath: string): ProjectDocument {
  ...
  const meta: ProjectMeta = {
    ...
    visibility: asVisibility(parsed.data.visibility ?? parsed.data.publishStatus),
  };
  return { meta, body: parsed.content.trim(), absolutePath, assetBasePath: path.dirname(absolutePath) };
}
...
export async function getAllProjects({ includeDrafts = false }: { includeDrafts?: boolean } = {}) {
