...
async function readEntry(filePath: string, slug: string): Promise<ContentEntry | null> {
...
function leaf(label: string, href: string, activeHref?: string): ContentTreeNode {
...
function routeFromParts(parts: string[], slug = parts.at(-1) ?? "") {
  ...
  if (routeParts[0] === "pages") return `/${routeParts.slice(1).join("/") || ""}` || "/";
...
function nodeIsActive(node: ContentTreeNode, activeHref?: string): boolean {
  ...
  return Boolean(node.children?.some((child) => nodeIsActive(child, activeHref)));
}
...
async function buildDirectoryNode(dirPath: string, parts: string[], activeHref?: string): Promise<ContentTreeNode | null> {
    ...
    await Promise.all(
      dirEntries.map(async (entry) => {
        ...
        if (entry.isDirectory()) {
          return buildDirectoryNode(entryPath, [...parts, entry.name], activeHref);
        }
        ...
        return leaf(contentEntry.title, routeFromParts([...parts, slug], contentEntry.slug), activeHref);
      }),
    )
  ).filter((node): node is ContentTreeNode => Boolean(node)).sort(sortNodes);
  ...
  return { ...node, defaultOpen: node.defaultOpen || nodeIsActive(node, activeHref) };
}

export async function buildContentTree(activeHref?: string): Promise<ContentTreeNode[]> {
  const rootChildren = [leaf("home", "/", activeHref)];
  if (isAdminVisible()) rootChildren.push(leaf("cms.local", "/admin", activeHref));
  ...
  const contentNodes = (
    await Promise.all(
      entries.map(async (entry) => {
        ...
        if (entry.isDirectory()) return buildDirectoryNode(entryPath, [entry.name], activeHref);
        if (!entry.isFile() || !isMarkdownFile(entry.name)) return null;

        const slug = entry.name.replace(/\.(md|mdx)$/i, "");
        const contentEntry = await readEntry(entryPath, slug);
        if (!contentEntry) return null;
        return leaf(contentEntry.title, routeFromParts([contentEntry.slug]), activeHref);
      }),
    )
  ).filter((node): node is ContentTreeNode => Boolean(node)).sort(sortNodes);
