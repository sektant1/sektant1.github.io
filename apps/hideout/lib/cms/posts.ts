...
import { normalizePostPayload, postDocumentToPayload, serializePost } from "@/lib/cms/validation";
...
export async function createCmsPost(input: unknown) {
  ...
  await fs.writeFile(postIndexPath(payload.slug), serializePost(payload), "utf8");
...
export async function updateCmsPost(currentSlug: string, input: unknown) {
  ...
  await preserveSidecarFiles(postDir(currentSlug), postDir(payload.slug), async () => {
    if (payload.slug !== currentSlug) {
      const slugConflict = await getPostBySlug(payload.slug, { includeDrafts: true });
      if (slugConflict) throw new Error(`Slug already exists: ${payload.slug}`);
